...
import org.kpn.ch15.services.SingerService;
...
@Component
@ManagedResource(description = "JMX managed resource", objectName = "jmxDemo:name=KPNApp")
public class AppStaticsImpl implements AppStatistics{
    ...
    @Override
    public int getTotalSingerCounter() {
    ...
    public String findByFirstName(String firstName) {
        List<Singer> singers = singerService.findByFirstName(firstName);
        if (!singers.isEmpty()){
            return singers.get(0).getFirstName() + " " + singers.get(0).getLastName();
        }
        return "not found";

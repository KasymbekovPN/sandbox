...
public class JobExecutionStatsListener extends JobExecutionListenerSupport {
    ...
    private static final Logger logger = LoggerFactory.getLogger(JobExecutionStatsListener.class);
    ...
    public void afterJob(JobExecution jobExecution) {
        if (jobExecution.getStatus() == BatchStatus.COMPLETED){
            ...
            jdbcTemplate.query(
                    "select first_name, last_name, song from singer",
                    (rs, row) -> new Singer(
                            rs.getString(1),
                            rs.getString(2),
                            rs.getString(3)
                    )
            ).forEach(singer -> logger.info(singer.toString()));
        }
    }
}
